#>jinro_rpg:game/wins/villagerwin


##Debugメッセージ
    #tellraw @a[tag=op] "villagerwin"


##タイトル
    execute as @a at @s run title @s title [{"color":"white","text":"☻ "},{"bold":true,"color":"blue","text":"村人の勝利"}," ☻"]
...
##勝利陣営
    ...
    execute if entity @a[tag=Clown] run tellraw @a [{"text":"・"},{"bold":true,"color":"#FF8C00","text":"道化師"}]
    execute if entity @a[tag=Clown] as @a[tag=Clown] at @s run function jinro_rpg:game/wins/get_info


##敗北陣営
    tellraw @a {"bold":true,"color":"red","text":"敗北陣営"}
    execute if entity @a[tag=Wolf] run tellraw @a [{"text":"・"},{"bold":true,"color":"dark_red","text":"人狼"}]
    execute if entity @a[tag=Wolf] as @a[tag=Wolf] at @s run function jinro_rpg:game/wins/get_info
